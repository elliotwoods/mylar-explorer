...
export function createVolumetricControls(gui, params, hooks, statsModel) {
  ...
  const pairScopeController = inject.add(
    params.volumetrics,
    "pairInjectionScope",
    { "Reflected only": "reflected", "Reflected + incident": "reflected+incident" }
  );
  pairScopeController.name("Pair Scope");
  pairScopeController.domElement.title = "Inject between V-adjacent pairs for reflected-only beams or for both reflected and incident beams.";
  pairScopeController.onChange(hooks.onVolumetricConfigChange);
  ...
  stats.add(statsModel, "pairCountReflected").name("Pairs Reflected").listen();
  stats.add(statsModel, "pairCountIncident").name("Pairs Incident").listen();
  stats.add(statsModel, "pairCountInjected").name("Pairs Injected").listen();
